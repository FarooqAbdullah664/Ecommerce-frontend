    ...
    return (
        <Box sx={{ bgcolor: "#F8F9FA", minHeight: "100vh" }}>
            ...
            <Container maxWidth="xl" sx={{ py: { xs: 3, md: 5 }, px: { xs: 2, sm: 3 } }}>
                {/* Filters */}
                <Box sx={{ display: "flex", gap: { xs: 1, md: 2 }, mb: { xs: 3, md: 4 }, flexWrap: "wrap", alignItems: "center", justifyContent: "space-between" }}>
                    <Box display="flex" gap={1} flexWrap="wrap">
                        {categories.map((c) => (
                            <Chip key={c} label={c} onClick={() => { setCategory(c); setPage(1); }}
                                sx={{ fontWeight: 600, cursor: "pointer", borderRadius: 2, height: { xs: 30, md: 34 }, fontSize: { xs: "0.7rem", md: "0.8125rem" }, bgcolor: category === c ? "#1B2B4B" : "white", color: category === c ? "white" : "#6B7280", border: `1px solid ${category === c ? "#1B2B4B" : "#E5E7EB"}`, transition: "all 0.2s", "&:hover": { bgcolor: category === c ? "#0f1c33" : "#F8F9FA", borderColor: "#1B2B4B" } }}
                            />
                        ))}
                        {(search || category !== "All") && (
                            <Chip label="Clear" onClick={() => { setSearch(""); setCategory("All"); setPage(1); }} sx={{ fontWeight: 600, cursor: "pointer", borderRadius: 2, height: { xs: 30, md: 34 }, bgcolor: "#fff5f5", color: "#FF6B6B", border: "1px solid #fecaca" }} />
                        )}
                    </Box>
                    <FormControl size="small" sx={{ minWidth: { xs: 130, md: 170 } }}>
                                    ...
                                    <Grid item xs={6} sm={6} md={4} lg={3} key={p._id}>
                                        <Card sx={{ height: "100%", display: "flex", flexDirection: "column", "&:hover": { transform: "translateY(-8px)", boxShadow: "0 20px 50px rgba(27,43,75,0.12)", borderColor: "#1B2B4B", "& .overlay-btns": { opacity: 1, transform: "translateY(0)" }, "& .pimg": { transform: "scale(1.06)" } } }}>
                                            <Box sx={{ position: "relative", overflow: "hidden", bgcolor: "#F8F9FA" }}>
                                                <Box className="pimg" component="img" src={p.image || `https://placehold.co/400x230/EEF2FF/1B2B4B?text=${encodeURIComponent(p.name)}`} alt={p.name} sx={{ width: "100%", height: { xs: 160, sm: 200, md: 230 }, objectFit: "cover", display: "block", transition: "transform 0.4s" }} />
                                            ...
                                            <CardContent sx={{ flexGrow: 1, p: { xs: 1.5, md: 2.5 }, pb: 1 }}>
                                                ...
                                                <Typography fontWeight={700} color="#1B2B4B" sx={{ fontSize: { xs: "0.8rem", md: "1rem" }, overflow: "hidden", display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", lineHeight: 1.4, mb: 0.5 }}>{p.name}</Typography>
                                                {p.brand && <Typography variant="caption" color="text.secondary" sx={{ display: { xs: "none", sm: "block" } }}>by {p.brand}</Typography>}
                                                ...
                                                <Box display="flex" alignItems="center" gap={0.8} mt={0.5} sx={{ display: { xs: "none", sm: "flex" } }}>
                                            ...
                                            <CardActions sx={{ px: { xs: 1.5, md: 2.5 }, pb: { xs: 1.5, md: 2.5 }, pt: 1, gap: 1 }}>
                                                ...
                                                <Button size="small" variant="contained" fullWidth disabled={p.stock === 0} startIcon={<ShoppingCartIcon sx={{ fontSize: "12px !important", display: { xs: "none", sm: "inline" } }} />} onClick={() => handleAddToCart(p)} sx={{ borderRadius: 2, bgcolor: "#1B2B4B", fontSize: { xs: "0.7rem", md: "0.8rem" }, "&:hover": { bgcolor: "#0f1c33" } }}>{p.stock === 0 ? "Sold Out" : "Add"}</Button>
                                            </CardActions>
                                        </Card>
                                    </Grid>
                                );
